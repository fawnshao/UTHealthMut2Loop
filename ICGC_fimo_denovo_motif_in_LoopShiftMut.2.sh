...
awk '{print $1"\t"$3}' $pre.LoopBroken.bed | grep -wf - $pre.rawmut.txt > $pre.LoopBroken.mut.txt
...
grep ">" $pre.ref.fa  | cut -d">" -f 3 | awk -F"." '{print $2"\t"$3-1}' | grep -wf - $pre.LoopBroken.bed | awk '{print $1":"$3"\t"$4}' > $pre.a
echo -n "" > $pre.b
grep ">" $pre.ref.fa  | cut -d">" -f 3 | awk -F"." '{print $2":"$3}' | while read line
do
	grep -w $line $pre.a | awk '{print ">"$2"\n"}' >> $pre.b
done
