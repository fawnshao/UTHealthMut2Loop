...
# perl $myperl gencode.v19.all.transcript.appris_principal.leve12.tss.bed hkg.tsg.srtbyPCA.class 3 0 > hkg.tsg.srtbyPCA.class.appris.tss
...
# gunzip -c gencode.v19.annotation.gtf.gz | awk '$3=="transcript"' | grep -vw -e "appris_principal" -e "appris_candidate" | awk -vOFS="\t" '{print $1,$4,$5,$10"|"$12,".",$7}' | sed 's/"//g;s/;//g' > gencode.v19.transcript.noappris.bed
# gunzip -c gencode.v19.annotation.gtf.gz | awk '$3=="transcript"' | grep -w -e "appris_principal" -e "appris_candidate" -e "appris_candidate_longest" | awk -vOFS="\t" '{print $1,$4,$5,$10"|"$12,".",$7}' | sed 's/"//g;s/;//g' > gencode.v19.transcript.appris.pcc.bed
# gunzip -c gencode.v19.annotation.gtf.gz | awk '$3=="transcript"' | grep -e "appris" | awk -vOFS="\t" '{print $1,$4,$5,$10"|"$12,".",$7}' | sed 's/"//g;s/;//g' > gencode.v19.transcript.appris.all.bed
# for f in gencode.v19.transcript.appris.*
# do
# 	echo -n $f": "
# 	cut -f 4 $f | cut -d"|" -f 1 | uniq | wc -l
# done
# gencode.v19.transcript.appris.all.bed: 20738
# gencode.v19.transcript.appris.bed: 20607
# gencode.v19.transcript.appris.pcc.bed: 20738


##################################################
# gunzip -c gencode.v19.annotation.gtf.gz | awk '$3=="transcript"' | grep -w "basic" | grep -w -e "appris_principal" -e "appris_candidate" -e "appris_candidate_longest" | awk -vOFS="\t" '{print $1,$4,$5,$10"|"$12,".",$7}' | sed 's/"//g;s/;//g' > gencode.v19.transcript.appris.annotated.bed
# gunzip -c gencode.v19.annotation.gtf.gz | awk '$3=="transcript"' | grep -w "basic" | grep -vw -e "appris_principal" -e "appris_candidate" -e "appris_candidate_longest" | awk -vOFS="\t" '{print $1,$4,$5,$10"|"$12,".",$7}' | sed 's/"//g;s/;//g' | sed 's/|/\t/' | perl $myperl <(cut -f 4 gencode.v19.transcript.appris.annotated.bed | cut -d"|" -f 1 | sort | uniq) /dev/stdin 0 3 | awk -vOFS="\t" '$8=="/"{print $1,$2,$3,$4"|"$5,$6,$7}' > gencode.v19.transcript.appris.notannotated.bed
# # | grep -vf <(cut -f 4 gencode.v19.transcript.appris.annotated.bed | cut -d"|" -f 1 | sort | uniq) > gencode.v19.transcript.appris.notannotated.bed
# cat gencode.v19.transcript.appris.annotated.bed gencode.v19.transcript.appris.notannotated.bed | bedtools sort -i - > gencode.v19.transcript.appris.first.bed
##################################################

# ln -s gencode.v19.transcript.appris.bed gencode.v19.transcript.bed
ln -s gencode.v19.transcript.appris.first.bed gencode.v19.transcript.bed
...
awk -F"\t" -vOFS="\t" '$2~/hkg/{print $3,$4,$5,$6,$7,$8}' hkg.tsg.srtbyPCA.transcript.bed > HKG.transcript.bed
awk -F"\t" -vOFS="\t" '$2!~/hkg/ && $2!~/mixTSG/{print $3,$4,$5,$6,$7,$8}' hkg.tsg.srtbyPCA.transcript.bed > singleTSG.transcript.bed
awk -F"\t" -vOFS="\t" '$2~/mixTSG/{print $3,$4,$5,$6,$7,$8}' hkg.tsg.srtbyPCA.transcript.bed > mixTSG.transcript.bed


for gene in HKG.transcript.bed singleTSG.transcript.bed mixTSG.transcript.bed
do
	pre=`echo $gene | awk -F"." '{print $1}'`
	findMotifsGenome.pl <(awk -vOFS="\t" '{a=$2-300;b=$2+100;if($6=="-"){a=$3-100;b=$3+300}if(a<0){a=0;}print $1,a,b,$4,"1000",$6}' $gene) hg19 $pre.homer.motifs -size given > $pre.findMotifsGenome.txt &
	annotatePeaks.pl <(awk -vOFS="\t" '{a=$2-300;b=$2+100;if($6=="-"){a=$3-100;b=$3+300}if(a<0){a=0;}print $1,a,b,$4,"1000",$6}' $gene) hg19 -m /home1/04935/shaojf/myTools/HOMER/data/knownTFs/vertebrates/known.motifs -size given > $pre.vert.known.motifs.txt &
done
wait

cat HKG.vert.known.motifs.txt <(tail -n +2 singleTSG.vert.known.motifs.txt) <(tail -n +2 mixTSG.vert.known.motifs.txt) | cut -f 1,22- | perl ~/myTools/UTHealthMut2Loop/relatedScripts/make.matrix.from.homer.motif.file.pl /dev/stdin > hkg.tsg.srtbyPCA.transcript.vert.known.motifs.mat

perl $myperl hkg.tsg.srtbyPCA.transcript.vert.known.motifs.mat hkg.tsg.srtbyPCA.transcript.class 0 2 | cut -f 1-3,5- > hkg.tsg.srtbyPCA.transcript.class.vert.known.motifs.mat

cut -f 1 HKG.homer.motifs/knownResults.txt | head -31 > HKG.top30.motif
perl $myperl HKG.homer.motifs/knownResults.txt HKG.top30.motif 0 0 | cut -f 1,8 | perl $myperl singleTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-2,9 | perl $myperl mixTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-3,10 > HKG.top30.motif.percentage
perl $myperl HKG.homer.motifs/knownResults.txt HKG.top30.motif 0 0 | cut -f 1,6 | perl $myperl singleTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-2,7 | perl $myperl mixTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-3,8 > HKG.top30.motif.qvalue

head_line hkg.tsg.srtbyPCA.transcript.class.vert.known.motifs.mat | grep -wf <(awk -F"/" '{print $1"/"$2}' HKG.top30.motif) | cut -d" " -f 1 | tr "\n" "," | sed 's/,$//' | xargs -I mycol cut -f1-3,mycol hkg.tsg.srtbyPCA.transcript.class.vert.known.motifs.mat > hkg.tsg.srtbyPCA.transcript.class.top30.vert.known.motifs.mat

# cut -f 1 HKG.homer.motifs/knownResults.txt | head -51 > HKG.top50.motif
# perl $myperl HKG.homer.motifs/knownResults.txt HKG.top50.motif 0 0 | cut -f 1,8 | perl $myperl singleTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-2,9 | perl $myperl mixTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-3,10 > HKG.top50.motif.percentage
# perl $myperl HKG.homer.motifs/knownResults.txt HKG.top50.motif 0 0 | cut -f 1,6 | perl $myperl singleTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-2,7 | perl $myperl mixTSG.homer.motifs/knownResults.txt /dev/stdin 0 0 | cut -f 1-3,8 > HKG.top50.motif.qvalue
