...
args <- c("hkg.tsg.srtbyPCA.transcript.tpm", 
	"hkg.tsg.srtbyPCA.transcript.peaks", 
	"hkg.tsg.srtbyPCA.transcript.loop", 
	"hkg.tsg.srtbyPCA.transcript.class")
tpm <- fread(args[1], sep = "\t", header = T, na.strings = "/")
chip <- fread(args[2], sep = "\t", header = T, na.strings = "/")
loop <- fread(args[3], sep = "\t", header = T, na.strings = "/")
class <- fread(args[4], sep = "\t", header = T, na.strings = "/")

genes <- as.matrix(class[,3])
types <- factor(as.matrix(class[,2]))
types.sim <- as.vector(types)
types.sim[1:3662] <- "HKG"
types.sim[3663:6084] <- "singleTSG"
# types.sim[1:26243] <- "HKG"
# types.sim[26244:37634] <- "singleTSG"
...
### for new data.table
#### transcript tpm visualization
annos <- data.frame(class = types)
rownames(annos) <- genes
colors <- colorRampPalette(c("blue", "white", "red"))(100)

# scores <- data.matrix(tpm[,-1])
# rownames(scores) <- genes
# a <- colnames(scores)
# scores.x <- data.frame(scores[, grep(pattern="^[A-Z]", a)], scores[, grep(pattern="^[a-z]", a)])
# data.x <- log2(scores.x + 1)
# data.x[data.x > 10] <- 10
# png(filename = paste(args[1], "pheatmap.png", sep = "."), width = 2500, height = 1600)
# myplot <- pheatmap(data.x, scale = "none", annotation_row = annos,
# 	show_rownames = F, show_colnames = T, color = colors, gaps_row = c(3662, 6084),
# 	cluster_cols = F, cluster_rows = F)
# dev.off()

Plotheatmap <- function(x, y = annos, z = colors, pre = args[1]){
	a <- colnames(x)
	scores.x <- data.frame(x[, grep(pattern="^[A-Z]", a)], x[, grep(pattern="^[a-z]", a)])
	png(filename = paste(pre, "pheatmap.png", sep = "."), width = 2500, height = 1600)
	myplot <- pheatmap(scores.x, scale = "none", annotation_row = y,
...
scores <- data.matrix(tpm[,-1])
rownames(scores) <- genes
data.x <- log2(scores + 1)
data.x[data.x > 10] <- 10
Plotheatmap(data.x, pre = args[1])

scores <- data.matrix(chip[,-1])
rownames(scores) <- genes
scores[is.na(scores)] <- 0
data.x <- apply(scores, 2, range01)
Plotheatmap(data.x, pre = args[2])

scores <- data.matrix(chip[,-1])
rownames(scores) <- genes
a <- colnames(scores)
# hist <- a[c(grep(pattern="H3K", a),grep(pattern="H4K", a),grep(pattern="H2AK", a),grep(pattern="H2BK", a))]
# tf <- a[-c(grep(pattern="H3K", a),grep(pattern="H4K", a),grep(pattern="H2AK", a),grep(pattern="H2BK", a))]
hist <- c(grep(pattern="H3K", a),grep(pattern="H4K", a),grep(pattern="H2AK", a),grep(pattern="H2BK", a))
scores.x <- scores[, -hist]
scores.x[is.na(scores.x)] <- 0
data.x <- apply(scores.x, 2, range01)
Plotheatmap(data.x, pre = "encode.tf")
scores.x <- scores[, hist]
scores.x[is.na(scores.x)] <- 0
data.x <- apply(scores.x, 2, range01)
Plotheatmap(data.x, pre = "encode.hist")

scores <- data.matrix(loop[,-1])
rownames(scores) <- genes
scores[is.na(scores)] <- 0
data.x <- apply(scores, 2, range01)
Plotheatmap(data.x, pre = args[3])

# conda install -c conda-forge r-d3heatmap 
# library(d3heatmap)
# png(filename = paste(args[1], "pheatmap.png", sep = "."), width = 2500, height = 1600)
# d3heatmap(data.x, Rowv = Rowv, Colv = Colv)
# dev.off()
