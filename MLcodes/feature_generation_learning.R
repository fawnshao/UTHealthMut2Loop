...
ggplot(data = mydata, aes(x = group, y = importance)) +
	...
	ggtitle("barplot of HiC.HiChIP.GM.Liver.CAGE.Homer") +
...
for(i in columns){
	...
	myplot <- ggplot(data = datax, aes(x = type, y = count, fill = type)) + 
		geom_boxplot(position = position_dodge(1), outlier.alpha = 0.1, outlier.size = 0.1) + 
		ggtitle(paste("count of", colnames(input)[i])) + theme(legend.position = "none")
	print(myplot)
	dev.off()
}
