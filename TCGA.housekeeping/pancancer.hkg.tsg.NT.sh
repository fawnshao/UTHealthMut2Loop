...
cut -f 1,102 v1.5.T.TSG.tsv | grep -v "," | sort -k2 | grep -v "tissueflags" > v1.5.T.TSG.sim
cut -f 1,102 v1.5.T.TSG.tsv | grep "," | sort -k2 >> v1.5.T.TSG.sim 
cut -f 1,2 v1.5.T.HKG.median.pheatmap.tsv | tail -n +2 | awk '{print $2"\thkg"$1}' | sort -k2 > v1.5.T.HKG.sim 
head -1 v1.5.T.log2tpm.median.tsv | cut -f2- | awk '{print "Gene\tFlags\t"$0}' > pancancer.T.hkg.tsg.tsv
perl $myperl v1.5.T.log2tpm.median.tsv <(cat v1.5.T.HKG.sim v1.5.T.TSG.sim) 0 0 | cut -f 1-2,4- >> pancancer.T.hkg.tsg.tsv

cut -f 1,39 v1.5.N.TSG.tsv | grep -v "," | sort -k2 | grep -v "tissueflags" > v1.5.N.TSG.sim
cut -f 1,39 v1.5.N.TSG.tsv | grep "," | sort -k2 >> v1.5.N.TSG.sim 
cut -f 1,2 v1.5.N.HKG.median.pheatmap.tsv | tail -n +2 | awk '{print $2"\thkg"$1}' | sort -k2 > v1.5.N.HKG.sim 
head -1 v1.5.N.log2tpm.median.tsv | cut -f2- | awk '{print "Gene\tFlags\t"$0}' > pancancer.N.hkg.tsg.tsv
perl $myperl v1.5.N.log2tpm.median.tsv <(cat v1.5.N.HKG.sim v1.5.N.TSG.sim) 0 0 | cut -f 1-2,4- >> pancancer.N.hkg.tsg.tsv

perl $myperl <(cat v1.5.N.HKG.sim v1.5.N.TSG.sim) <(cat v1.5.?.??G.sim | cut -f 1 | sort | uniq) 0 0 | cut -f 1,3 | perl $myperl <(cat v1.5.T.HKG.sim v1.5.T.TSG.sim) /dev/stdin 0 0 | cut -f 1-2,4 > pancancer.NT.hkg.tsg.sim
...
paste <(head -1 v1.5.N.log2tpm.median.tsv | cut -f2- | awk '{print "Gene\tN.Flags\tT.Flags\t"$0}') <(head -1 v1.5.T.log2tpm.median.tsv | cut -f2-) > pancancer.NT.hkg.tsg.tsv
paste v1.5.N.log2tpm.median.tsv <(cut -f 2- v1.5.T.log2tpm.median.tsv) | perl $myperl /dev/stdin pancancer.NT.hkg.tsg.sim.srt 0 0 | cut -f 1-3,5- >> pancancer.NT.hkg.tsg.tsv
...
# annosR[N.Flags == "hkg1" | N.Flags == "hkg2" | N.Flags == "hkg3" | N.Flags == "hkg4" | N.Flags == "hkg5", 1] <- "HKG"
# annosR[T.Flags == "hkg1" | T.Flags == "hkg2" | T.Flags == "hkg3" | T.Flags == "hkg4" | T.Flags == "hkg5", 2] <- "HKG"
...
cat <(cut -f 1 -d"." pancancer.NT.hkg.tsg.tsv | tail -n +2) <(cut -f 1 -d"." hkg.tsg.srtbyPCA.class | tail -n +2) | sort | uniq > hkg.tsg.id
echo "gene GTEx pancancer.N pancancer.T" | tr " " "\t" > hkg.tsg.cat
perl $myperl <(tail -n +2 hkg.tsg.srtbyPCA.class | sed 's/\./\t/' | cut -f 1,3) hkg.tsg.id 0 0 | cut -f 1,3 | perl $myperl <(tail -n +2 pancancer.NT.hkg.tsg.tsv | sed 's/\./\t/' | cut -f 1,3-4) /dev/stdin 0 0 | cut -f 1-2,4- | sort -k2 >> hkg.tsg.cat
...
paste <(head -1 v1.5.N.log2tpm.median.tsv) <(head -1 v1.5.T.log2tpm.median.tsv | cut -f 2-) > sim.v1.5.NT.log2tpm.median.tsv
paste <(tail -n +2 v1.5.N.log2tpm.median.tsv | cut -f 1 -d".") <(tail -n +2 v1.5.N.log2tpm.median.tsv | cut -f 2-) <(tail -n +2 v1.5.T.log2tpm.median.tsv | cut -f 2-) >> sim.v1.5.NT.log2tpm.median.tsv
...
perl $myperl sim.v1.5.NT.log2tpm.median.tsv hkg.tsg.cat.srt 0 0 | cut -f 1-4,6- > hkg.tsg.cat.srt.pancancer
...
# annosR[pancancer.N == "hkg1" | pancancer.N == "hkg2" | pancancer.N == "hkg3" | pancancer.N == "hkg4" | pancancer.N == "hkg5", 2] <- "HKG"
# annosR[pancancer.T == "hkg1" | pancancer.T == "hkg2" | pancancer.T == "hkg3" | pancancer.T == "hkg4" | pancancer.T == "hkg5", 3] <- "HKG"
# annosR[grep(",",pancancer.N),2] <- "mixTSG"
# annosR[grep(",",pancancer.T),3] <- "mixTSG"
# scores[is.na(scores) | scores < 0] <- 0

# gtex.mean <- apply(scores[,1:49], 1, mean)
# pancancer.mean <- apply(scores[,60:90], 1, mean)
...
# scores.diff.hkg.anno <- rbind.data.frame(scores.diff.hkg.1, scores.diff.hkg.2, scores.diff.hkg.3, scores.diff.hkg.4)[,91:94]
...
# png(filename = paste(args[1], "pheatmap.png", sep = "."), width = 1500, height = 1200)
# myplot <- pheatmap(scores, scale = "none", annotation_row = annosR, 
# 	show_rownames = F, show_colnames = T, color = colors, gaps_col = c(49,59),
# 	cluster_cols = F, cluster_rows = F)
# dev.off()
...
# 	show_rownames = T, show_colnames = T, color = colors, fontsize_row = 1, gaps_col = c(49,59),
# 	cluster_cols = F, cluster_rows = F)
# dev.off()
...
perl $myperl <(awk -F"\t" -vOFS="\t" '($14=="Active" || $15=="Active") && $16==""{print $4,$1,$14,$15}' Dao.2017ng.tableS2.txt) <(perl $myperl sim.gencode.v23.annotation.gene.probeMap hkg.tsg.cat.srt.GTEx.pancancer.diff.hkg.sim 0 0 | cut -f 1-5,7) 0 5 > hkg.tsg.cat.srt.diff.Epromoters
