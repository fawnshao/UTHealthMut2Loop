...
do
	...
	perl $myperl rand273.200kb.nonpseudoncogene.neighbors.stats \
		...
		cut -f 1-3,5 > rand273.200kb.nonpseudoncogene.neighbors.pMUT.forPermutation.txt
	Rscript wilcox.test.R oncogene.200kb.nononcogene.neighbors.pMUT.forPermutation.txt \
		rand273.200kb.nonpseudoncogene.neighbors.pMUT.forPermutation.txt onco.pvals.txt
	i=$[$i+1]
done

awk '$2<0.05' onco.pvals.txt | cut -f 1 | sort | uniq -c
