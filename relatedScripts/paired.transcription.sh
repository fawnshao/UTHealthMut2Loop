...
perl ~/stampede2/myTools/UTHealthMut2Loop/relatedScripts/get.nearest.pairs.pl gencode.appris_principal.leve12.tss.neighbors.txt > uniq.tss.pairs

# mytss=gencode.appris_principal.leve12.tss.neighbors.txt
mytss=uniq.tss.pairs
perl $myperl <(cut -f 4,6,10,12,13 $mytss) hkg.tsg.srtbyPCA.class 0 0 | cut -f 1-2,4- | perl $myperl genecode.gene.all.annotation.txt /dev/stdin 0 0 | cut -f 1-6,8 | perl $myperl genecode.gene.all.annotation.txt /dev/stdin 0 3 | cut -f 1-7,9 | perl $myperl hkg.tsg.srtbyPCA.class /dev/stdin 0 0 | cut -f 1-8,10 > hkg.tsg.srtbyPCA.class.appris.tss.neighbors.sim.annotation
